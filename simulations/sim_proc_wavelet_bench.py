...
zf = 2**18 #optional larger 0-fill

##Get reference with no noise
R2 = 0.08 #fake R2 global, unitless
name = 'Sn_CPMG_id_300.fid'
...
for i in range(kk):
    ...
    specrecon[:,i], coeffin, coeffs = wave.wavelet_denoise(3, np.real(spec), 0, wave = 'bior2.2', threshold = 'mod', alpha = 0)
...
data=[]
for i in range(len(snrpin)):
    data.append( ["%.1f"%snrpin[i],"%.1f"%snrpout[i], "%.4f"%SSIMin[i], "%.4f"%SSIMout[i],"%.4f"%SSIMinlow[i], "%.4f"%SSIMoutlow[i]] )
# create header
